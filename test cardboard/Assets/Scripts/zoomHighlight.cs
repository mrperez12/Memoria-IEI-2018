...
public class zoomHighlight : MonoBehaviour {
	...
	void Update () {
		if (myToggle.isOn) {
			scene.color = new Color32 (120, 120, 120, 255);
		} else {
			scene.color = new Color32 (255, 255, 255, 255);
		}
	}
	/*Funcion que se llama al entrar la mira sobre el objeto con event trigger*/
	public void PointerEnter(){

		scene.transform.position = new Vector3 (scene.transform.position.x, scene.transform.position.y, scene.transform.position.z - 0.5f);
